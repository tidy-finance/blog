...
For the most part, [Tidy Finance with R](https://www.tidy-finance.org/chapters/index.html) uses SQLite for storage and uses `dplyr` and in-memory data frames for analysis.
For example, in the [chapter on beta estimation](https://www.tidy-finance.org/chapters/beta-estimation.html), the data are read into memory immediately using `collect()` before any analysis is conducted.
...
The following code is adapted from the Tidy Finance code [here](https://www.tidy-finance.org/chapters/wrds-crsp-and-compustat.html).
