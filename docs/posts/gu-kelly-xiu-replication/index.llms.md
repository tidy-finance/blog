...
characteristics <- read_csv(archive_read(
...
macro_predictors <- macro_predictors |>
  select(month) |>
  left_join(
    macro_predictors |> mutate(month = month %m+% months(1)),
    join_by(month)
  )
...
rec <- recipe(ret_excess ~ ., data = characteristics |> head()) |>
  ...
  step_interact(
    terms = ~ contains("characteristic"):contains("macro"),
...
visualization_data <- pmap_dfr(
  estimation_periods,
  \(training_start, training_end, validation_start, validation_end, oos_year) {
    tibble(
      year = 1957:2021,
      classification = case_when(
        year >= training_start & year <= training_end ~ "Training",
...
ggplot(
  ...
  geom_point(size = 1) +
  labs(
    title = "Data classification timeline",
    x = NULL,
    y = NULL,
    color = NULL
  ) +
...
``` r
characteristics_prepared <- open_dataset(
  sources = "data/characteristics_prepared"
)
```
...
validation <- characteristics_prepared |>
  filter(
    year(month) >= split_dates$validation_start &
...
folds <- manual_rset(list(data_split), ids = "split")
```
...
write_rds(
  fitted_workflow,
  file = paste0("data/fitted_workflow_", split_dates$oos_year, ".rds")
)
```
...
hml_portfolio <- ml_portfolios |>
  reframe(
    ...
    ret_predicted = ret_predicted[portfolio == 10] -
      ret_predicted[portfolio == 1],
...
ml_portfolios |>
  ...
  group_by(portfolio) |>
  summarize(
    predicted_mean = mean(ret_predicted),
