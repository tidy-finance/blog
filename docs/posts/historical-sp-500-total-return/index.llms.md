...
shiller_historical <- read_excel(temp, sheet = "Data", skip = 7) |>
  transmute(
    month = ceiling_date(
      ymd(str_replace(str_c(Date, ".01"), "\\.1\\.", "\\.10\\.")),
      "month"
    ) -
      1,
    price = as.numeric(P),
...
check <- shiller_historical |>
  ...
  arrange(month) |>
  mutate(
    ret = if_else(row_number() == 1, 0, ret), # ignore first month return
...
sp500_historical <- sp500_recent |>
  ...
  arrange(desc(month)) |>
  mutate(
    ret = if_else(row_number() == 1, 0, ret),
...
sp500_monthly |>
  ...
  labs(
    ...
    subtitle = glue(
      "Both indexes are normalized to 100 at {min(sp500_monthly$month)}"
    )
  )
```
